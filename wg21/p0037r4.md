**Document number**: LEWG, SG14, SG6: P0037R4  
**Date**: 2017-10-16  
...
* certain applications can suffer from the variability in precis`ion which comes from a dynamic radix point 
...
1. A `fixed_point<T, 0>` should follow the same behavior as `T` to the maximum
...
Function, `divide`, however serves a different purpose.
...
This poses a problem to the unwary user of `fixed_point` who may see the radix
point and assume that dividing `fixed_point` variables will yield similar results.
In particular, almost 50% of all integer and `fixed_point` division operations
contain a numerator greater than the denominator, resulting in a value of zero.
In another example, where both operands have the same exponent, the result will
have an exponent of zero.

The `divide` function meets the needs of users expecting results to contain a
number of integer and fractional digits totalling those of the operands as described in 
...
is that results will vary depending on the width of types such as `int` and `long`.
This is not a problem for an `elastic_integer` type
([\[P0554\]](http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2017/p0554r0.html#componentization))
...
`fixed_point` are the wrong way around:
...
The possibility of scaling other than an exponent has already been discussed
...
type, `scaled`:
    ...
    template<class Rep, int Exponent>
    using fixed_point = scaled<Rep, power<Exponent>>;
...
In this dichotomy, `scaled` would hold onto a member variable of type, `Rep`,
while `power` was stateless, providing an API which converted to and from
the `Rep` type through function parameters. `power` would also provide 
arithmetic support.

Alternatives to `power` would include:

* `ratio` (with additional arithmetic support)
